...
local function createLobby()
	...
	titleLabel.Text = "CASHFLOW EMPIRE"
	...
	subtitleLabel.Text = "Stack Cash, Build Empire, Rebirth!"
...
local function onPlayerAdded(player)
	...
	-- Update visit pad label and visibility for this plot
	if plotData.visitPad then
		local visitInfo = plotData.visitPad:FindFirstChild("VisitInfo")
		if visitInfo then
			local visitLabel = visitInfo:FindFirstChild("VisitLabel")
			if visitLabel then
				visitLabel.Text = "Visit " .. player.Name .. "'s Plot"
			end
			visitInfo.Enabled = true
		end
		plotData.visitPad.Transparency = 0
		plotData.visitPad.CanCollide = true
	end
...
local function onPlayerRemoving(player)
	...
	-- Reset visit pad (hide it since plot is no longer owned)
	if plotData.visitPad then
		plotData.visitPad.Transparency = 1
		plotData.visitPad.CanCollide = false
		local visitInfo = plotData.visitPad:FindFirstChild("VisitInfo")
		if visitInfo then
			visitInfo.Enabled = false
			local visitLabel = visitInfo:FindFirstChild("VisitLabel")
			if visitLabel then
				visitLabel.Text = "Visit Plot " .. plotNum
			end
